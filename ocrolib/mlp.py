...
nnet_native.backward_b.argtypes = [I,I,I,A2F,A1F,A2F,A1F, I,A2B,A1I,F,I]
...
class MLP:
    ...
    def changeHidden(self,data,cls,new_nhidden,subset=None):
        if nhidden==new_nhidden: return
        elif nhidden>new_nhidden: self.decreaseHidden(data,cls,new_nhidden)
    ...
    def train(self,data,cls,etas=[(0.1,100000)]*30,
              nhidden=None,eps=1e-2,subset=None,verbose=0):
        data = data.reshape(len(data),prod(data.shape[1:]))
        if subset is not None:
...
standard_etas = [(1.0,1000000)]*5 + [(0.1,1000000)]*20
...
if __name__=="__main__":
    ...
    print sum(classes)*1.0/len(classes)
    print bdata[:10],classes[:10]
    mlp = MLP()
    mlp.train(bdata[:9000,:],classes[:9000],verbose=1,nhidden=10,etas=[(1.0,100000)]*100)
